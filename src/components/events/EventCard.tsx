...
export const EventCard = ({ event, variant = "default" }: EventCardProps) => {
  ...
  // Check if registration link is external
  const isExternalLink = event.registrationLink?.startsWith("http");
      ...
      >
        ...
        <div className="relative h-full bg-card/90 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden flex flex-col">
          ...
          >
            ...
            {/* Footer - Updated with Clickable Registration Button */}
            <div className="mt-6 pt-4 border-t border-white/5 flex items-center justify-between text-sm font-montserrat font-semibold tracking-wider uppercase">
              ...
              {isExternalLink ? (
                <a
                  ...
                  className="flex items-center gap-2 text-muted-foreground hover:text-white transition-colors relative z-30"
                >
                  <span>Register</span>
                  <ArrowRight className="w-5 h-5 text-primary transform group-hover:translate-x-1 transition-transform" />
                </a>
                ...
                <Link
                  to={event.registrationLink || `/events/${event.id}/register`}
                  onClick={(e) => e.stopPropagation()}
                  className="flex items-center gap-2 text-muted-foreground hover:text-white transition-colors relative z-30"
                >
                  <span>Register</span>
